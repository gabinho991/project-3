...
def on_login(data):
    ...
    user = models.User(googleId=googleId,
                       email=email,
                       imageUrl=imageUrl,
                       givenName=givenName,
    ...
    ret = DB.session.query(
        exists().where(models.User.googleId == googleId)).scalar()
    if (ret is False):
        DB.session.add(user)
        DB.session.commit()
    ...
    currentUserInfo = DB.session.query(
        models.User).filter_by(googleId=googleId).first()
    personal_data = {
    ...
    socketio.emit('user_info',
                  [givenName, familyName, imageUrl, googleId, lst],
                  broadcast=True,
                  include_self=False)
